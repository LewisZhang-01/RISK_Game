...
public class Sprint2 {
	...
	private static void sequence(UI ui, int playerId) {
	...
	public static void main(String args[]) {
		...
		for (playerId = 0; playerId < GameData.NUM_PLAYERS; playerId++) {
			...
			int times = 3;// Players take it in turns to place 3 units at a time on a territory that they
							// control.
			while (times != 0) {

				ui.displayString(
						"Enter the name of territory which you want place unit. (player " + (playerId + 1) + ")");
				...
				while (unit_num <= 0 || unit_num > times) {
					ui.displayString("*You can only place 1(min) - " + times
							+ "(max) units at this time.\n*Please enter again: ");
					unit_num = Integer.parseInt(ui.getCommand());
				}
		...
		for (playerId = 0; playerId < GameData.NUM_NEUTRALS; playerId++) {
			int times = 1;
			while (times != 0) {
				ui.displayString("Enter the name of territory which you want place unit. (neutral player "
						+ (playerId + 1) + ")");
				territory = ui.getCommand();
				// invalid input handle: Letters only allowed.
				while (!territory.matches("[a-zA-Z]+")) {// check if user not enter letters.
